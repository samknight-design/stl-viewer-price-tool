...
- **Multi-model cart line items** — Task 9 flags this as a real v1 gap: only the first model group's file properties currently reach the actual Shopify order when an order has more than one model. Needs a follow-up before the packing tool can be trusted for multi-model orders.
