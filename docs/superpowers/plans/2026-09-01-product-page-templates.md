...
/* Dawn paints the quantity box's actual visible surface with a `.quantity:before`
   pseudo-element (background:rgb(var(--color-background)), positioned to cover
   the whole control) - the real <input> underneath is background-color:transparent
   per Dawn's own base.css, so overriding .quantity__input alone has no visible
   effect. Found live: the quantity box rendered as a plain white box despite
   .quantity__input carrying the right background rule. !important needed since
   Dawn's --color-background is scheme-based, not one of our tokens, and this
   rule needs to beat it outright rather than out-cascade it. */
...
/* Dawn's disabled/unavailable pill variants use rgba(var(--color-foreground),.6)
   for their (struck-through) label text - Dawn's own light-scheme foreground
   colour, unrelated to our tokens, so it's invisible against our dark pills.
   Found live: the "Assembled & unprimed" and other out-of-stock combinations
   were unreadable. !important for the same reason as .quantity:before above. */
product-info variant-selects .product-form__input--pill input[type="radio"]:disabled + label,
product-info variant-selects .product-form__input--pill input[type="radio"].disabled + label {
  color: var(--af-grey-600) !important;
  border-color: rgba(255, 255, 255, .15) !important;
}
product-info variant-selects .product-form__input--pill input[type="radio"]:disabled:checked + label,
product-info variant-selects .product-form__input--pill input[type="radio"].disabled:checked + label {
  color: var(--af-ink-900) !important;
}
```
